...
__global__ void assign_clusters(
    ...
    extern __shared__ float shared_centers[]; // size K_per_tile * D
    ...
    for (int k = 0; k < K; k += K_per_tile) {
        const int num_k = min(K_per_tile, K - k);

        for (int i = tid; i < num_k * D; i += blockDim.x) {
            shared_centers[i] = centers[k * D + i];
        }
        __syncthreads();
        ...
        if (idx < N) { // thread for each numpoint
            ...
            for (int j = 0; j < num_k; ++j) {
                float dist = 0.0f;
                for (int d = 0; d < D; ++d) {
                    float diff = points[idx * D + d] - shared_centers[j * D + d];
                    dist += diff * diff;
                }
                if (dist < min_dist) {
                    min_dist = dist;
...
__global__ void per_block_shared_reduction(
    ...
    int* counts_shared = reinterpret_cast<int*>(shared_data); // 0 to K_per_tile - 1 ints
    float* sums_shared = reinterpret_cast<float*>(counts_shared + K_per_tile); // memory starts after counts
    ...
    for (int k = 0; k < K; k += K_per_tile) {
        ...
        if (idx < N) {
            ...
            if (label >= k && label < k + num_k) { // belongs to k in this tile
                const int local_index = label - k;
                atomicAdd(&counts_shared[local_index], 1);
                for (int d = 0; d < D; ++d) {
                    atomicAdd(&sums_shared[local_index * D + d], points[idx * D + d]);
                }
            }
        }
        __syncthreads();
        ...
        for (int i = threadIdx.x; i < num_k; i += blockDim.x) {
            // counts_shared[i] (relative)
            // counts(k + i) (absolute)
            if (counts_shared[i] > 0) atomicAdd(&counts[k + i], counts_shared[i]);
        }
        for (int i = threadIdx.x; i < num_k * D; i += blockDim.x) {
            // sums_shared[i] (relative)
            // sums(k * D + i) (absolute)
            if (sums_shared[i] != 0.0f) atomicAdd(&sums[k * D + i], sums_shared[i]);
        }
        __syncthreads();
    }
}
...
int kmeans_cuda_shmem(
    ...
    while (iter_to_converge < max_iter) {
        ...
        int K_per_tile = 16; // number of clusters to load into shared memory at a time
        size_t shared_mem_size = K_per_tile * dims * sizeof(float);

        assign_clusters<<<blocks_numpoints, threads_per_block, shared_mem_size>>>(
            d_points,
            d_centers,
            d_labels,
            d_counts,
